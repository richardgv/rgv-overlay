...
inherit autotools eutils gnome2 python multilib git-2

DESCRIPTION="Diagram/flowchart creation program"
HOMEPAGE="http://live.gnome.org/Dia"
SRC_URI=""
EGIT_REPO_URI="git://git.gnome.org/dia"
...
pngfix() {
	local suffix=".fix-4e7ws2"
	for i in $@; do
		einfo "Fixing PNG file $i ..."
		pngcrush -q -fix -force "$i"{,$suffix} || die
		mv "$i"{$suffix,} || die
	done
}
...
src_prepare() {
	# Fix all PNGs
	find "${S}" -iname '*.png' | (while read -r i; do pngfix $i; done)

	# Fix obsolete AM_CONFIG_HEADER
	sed -i 's/AM_CONFIG_HEADER/AC_CONFIG_HEADER/' "${S}/configure.in" || die

	# Fix underlinking
	sed -i 's/\(GTK_MODULES="\)/\1gmodule-2.0 /' "${S}/configure.in" || die
